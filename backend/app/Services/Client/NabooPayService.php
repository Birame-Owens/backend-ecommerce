...
class NabooPayService
{
    ...
    public function createTransaction(Commande $commande, Paiement $paiement, string $provider, array $data = []): array
    {
        ...
        $payload = [
            ...
            'success_url' => $this->frontendUrl("/checkout/success?order={$commande->numero_commande}&t={$commande->access_token}"),
            'error_url' => $this->frontendUrl("/checkout/cancel?order={$commande->numero_commande}&t={$commande->access_token}"),
